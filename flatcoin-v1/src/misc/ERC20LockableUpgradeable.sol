...
contract ERC20LockableUpgradeable is Initializable, ERC20Upgradeable {
    ...
    function _lock(address account, uint256 amount) internal virtual {
        if (_lockedAmount[account] + amount > balanceOf(account)) {
            revert("ERC20LockableUpgradeable: locked amount exceeds balance");
        }
    ...
    function _unlock(address account, uint256 amount) internal virtual {
        if (_lockedAmount[account] < amount) {
            revert("ERC20LockableUpgradeable: requested unlock exceeds locked balance");
        }
