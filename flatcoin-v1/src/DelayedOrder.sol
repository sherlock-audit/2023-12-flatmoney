...
/*
    STABLE DEPOSIT FLOW (other orders work similarly)

    [User]
    |-> User calls 'announceStableDeposit()'
    |
    |----< minExecutabilityAge >----
    |
    |-> This is where a minimum time delay comes in to prevent frontrunning.
    |   The system needs an offchain oracle price after the minimum time for execution.
    |
    [Keeper]
    |-> Keeper executes 'executeStableDeposit()'
    |
    |----< maxExecutabilityAge >----
    |
    |-> Order expires and can be cancelled, or new order opened
*/
    ...
    OracleModifiers
{
    ...
    // TODO: Move the following to a settings contract.
    ...
    function announceStableDeposit(
        ...
        // TODO: Consider implementing announceDepositTo() with _announceDeposit() internal function as a common function for easier third party integrations
        ...
        // TODO: Make sure that the keepers can't be DDOSed in any way.
        // Ie. there should be a cost associated with announcing transactions (eg order cancellation fee)
    ...
    function announceStableWithdraw(
        ...
        // Check that the requested minAmountOut is feasible
        {
            uint256 expectedAmountOut = stableModule.stableWithdrawQuote(withdrawAmount) - keeperFee;

            // require(expectedAmountOut >= keeperFee, "DO: Not enough value for the keeper fee");
    ...
    function announceLeverageAdjust(
        ...
        int256 marginAfterAdjustment = int256(position.marginDeposited) + marginAdjustment;
        ...
        int256 newMargin = marginAdjustment > 0 ? marginAfterAdjustment : marginAfterAdjustment - int256(totalFee);
        // New additional size will be either bigger or smaller than current additional size, depends on if additionalSizeAdjustment is positive or negative.
        int256 newAdditionalSize = int256(position.additionalSize) + additionalSizeAdjustment;
        ...
        if (newMargin < 0 || newAdditionalSize < 0)
            revert FlatcoinErrors.ValueNotPositive("newMargin|newAdditionalSize");

        // New values can't be less than min margin and min/max leverage requirements
        leverageModule.checkLeverageCriteria(uint256(newMargin), uint256(newAdditionalSize));

        {
            int256 marginAfterSettlement = leverageModule.getPositionSummary(tokenId).marginAfterSettlement;
            int256 newMarginAfterSettlement = marginAfterSettlement +
                marginAdjustment -
                ((marginAdjustment > 0) ? int256(0) : int256(totalFee));

            if (
                newMarginAfterSettlement < 0 ||
                ILiquidationModule(vault.moduleAddress(FlatcoinModuleKeys._LIQUIDATION_MODULE_KEY))
                    .getLiquidationMargin(uint256(newAdditionalSize), fillPrice) >=
                uint256(newMarginAfterSettlement)
            ) revert FlatcoinErrors.PositionCreatesBadDebt();

            uint256 minimumMarginRequirement = leverageModule.marginMin();

            if (uint256(newMarginAfterSettlement) < minimumMarginRequirement)
                revert FlatcoinErrors.MarginTooSmall(minimumMarginRequirement, uint256(newMarginAfterSettlement));
        }
        ...
        _announcedOrder[msg.sender] = FlatcoinStructs.Order({
            ...
            orderData: abi.encode(
                FlatcoinStructs.AnnouncedLeverageAdjust({
                    ...
                    newMargin: uint256(newMargin),
                    newAdditionalSize: uint256(newAdditionalSize),
                    fillPrice: fillPrice,
    ...
    function _executeStableDeposit(address account) internal returns (uint256 liquidityMinted) {
        ...
        if (order.orderType != FlatcoinStructs.OrderType.StableDeposit)
            revert FlatcoinErrors.NoPendingOrderAllowed(order.orderType);
    ...
    function _executeLeverageOpen(address account) internal returns (uint256 tokenId) {
        ...
        if (order.orderType != FlatcoinStructs.OrderType.LeverageOpen)
            revert FlatcoinErrors.NoPendingOrderAllowed(order.orderType);
    ...
    function _executeLeverageAdjust(address account) internal {
        ...
        if (order.orderType != FlatcoinStructs.OrderType.LeverageAdjust)
            revert FlatcoinErrors.NoPendingOrderAllowed(order.orderType);
    ...
    function _executeLeverageClose(address account) internal returns (int256 settledMargin) {
        ...
        if (order.orderType != FlatcoinStructs.OrderType.LeverageClose)
            revert FlatcoinErrors.NoPendingOrderAllowed(order.orderType);
