...
contract PointsModule is ModuleUpgradeable, ERC20LockableUpgradeable {
    ...
    IFlatcoinVault public flatcoinVault;
    ...
    modifier onlyFlatcoinContracts() {
        if (flatcoinVault.isAuthorizedModule(msg.sender) == false)
            revert FlatcoinErrors.OnlyAuthorizedModule(msg.sender);
        _;
    }
    ...
    function mintLeverageOpen(address to, uint256 size) external onlyFlatcoinContracts {
    ...
    ///         The points start a 12 month unlock tax (update unlockTime).
    /// @dev The function will not revert if no points are minted because it's called by the flatcoin contracts.
    function mintDeposit(address to, uint256 depositAmount) external onlyFlatcoinContracts {
